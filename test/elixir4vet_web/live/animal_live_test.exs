defmodule Elixir4vetWeb.AnimalLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all animals", %{conn: conn, animal: animal} do
      {:ok, _index_live, html} = live(conn, ~p"/admin/animals")

      assert html =~ "Listing Animals"
      assert html =~ animal.name
    end

    test "saves new animal", %{conn: conn, user: user} do
      {:ok, index_live, _html} = live(conn, ~p"/admin/animals")

      assert {:ok, form_live, _} =
               index_live
               |> element("a", "New Animal")
               |> render_click()
               |> follow_redirect(conn, ~p"/admin/animals/new")
      ...
      assert {:ok, index_live, _html} =
               form_live
               |> form("#animal-form", animal: Map.put(@create_attrs, :owner_id, user.id))
               |> render_submit()
               |> follow_redirect(conn, ~p"/admin/animals")
    ...
    test "updates animal in listing", %{conn: conn, animal: animal} do
      ...
      assert {:ok, form_live, _html} =
               ...
               |> render_click()
               |> follow_redirect(conn, ~p"/admin/animals/#{animal}/edit")
  ...
  describe "Show" do
    ...
    test "displays animal", %{conn: conn, animal: animal} do
      {:ok, _show_live, html} = live(conn, ~p"/admin/animals/#{animal}")
    ...
    test "updates animal and returns to show", %{conn: conn, animal: animal} do
      {:ok, show_live, _html} = live(conn, ~p"/admin/animals/#{animal}")

      assert {:ok, form_live, _} =
               show_live
               |> element("a", "Edit")
               |> render_click()
               |> follow_redirect(conn, ~p"/admin/animals/#{animal}/edit?return_to=show")
      ...
      assert {:ok, show_live, _html} =
               ...
               |> form("#animal-form", animal: @update_attrs)
               |> render_submit()
               |> follow_redirect(conn, ~p"/admin/animals/#{animal}")
