defmodule Elixir4vetWeb.Admin.AnimalLive.Show do
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      ...
      <.list>
        <:item title={gettext("Name")}>{@animal.name}</:item>
        <:item title={gettext("Species")}>{@animal.species}</:item>
        <:item title={gettext("Breed")}>{@animal.breed}</:item>
        <:item title={gettext("Date of birth")}>{@animal.date_of_birth}</:item>
        <:item title={gettext("Microchip number")}>{@animal.microchip_number}</:item>
          ...
          <div class="flex gap-2 items-center">
            <form id="add-owner-form" phx-submit="add_owner" class="flex gap-2">
              ...
              <select name="ownership_type" class="select select-bordered select-sm">
                ...
                <option value="co-owner">{gettext("Co-owner")}</option>
                <option value="guardian">{gettext("Guardian")}</option>
                <option value="foster">{gettext("Foster")}</option>
              </select>
      ...
      <.table id="owners" rows={@owners}>
        <:col :let={{user, _type}} label={gettext("User")}>{user.email}</:col>
        <:col :let={{_user, type}} label={gettext("Type")}>{type}</:col>
        <:action :let={{user, type}}>
          <.link
            phx-click="remove_owner"
            phx-value-user_id={user.id}
            phx-value-ownership_type={type}
            data-confirm={gettext("Are you sure you want to remove this owner?")}
            class="text-error"
          >
            {gettext("Remove")}
          </.link>
  ...
  def handle_event("add_owner", %{"user_id" => user_id, "ownership_type" => type}, socket) do
    case Animals.add_animal_owner(
      ...
      {:ok, _ao} ->
        {:noreply,
         socket
         |> put_flash(:info, gettext("Owner added successfully"))
         |> refresh_owners()}

      {:error, _changeset} ->
        {:noreply, put_flash(socket, :error, gettext("Failed to add owner"))}
    end
  end
  ...
  def handle_event("remove_owner", %{"user_id" => user_id, "ownership_type" => type}, socket) do
    case Animals.remove_animal_owner(
      ...
      {:ok, _ao} ->
        {:noreply,
         socket
         |> put_flash(:info, gettext("Owner removed successfully"))
         |> refresh_owners()}
  ...
  def handle_info(
    ...
    {:noreply,
     socket
     |> put_flash(:error, gettext("The current animal was deleted."))
